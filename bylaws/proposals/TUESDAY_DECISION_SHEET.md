# TUESDAY LEADERSHIP DECISION SHEET

**Meeting:** September 22, 2026  
**Status:** Discussion document — none of these proposals are adopted unless leadership approves them.

The full rationale is in `PROPOSED_GOVERNANCE.md`. This sheet is intentionally short so the meeting can make concrete decisions.

---

## Decision 1 — Four-officer GSU structure

### Proposal

GSU elects:

- President
- Vice President
- Treasurer
- Secretary

The officers serve one-year terms.

### Recommendation

**Approve as proposed.**

### Why

This is familiar, easy for members to understand, and gives responsibility for executive leadership, succession, finances, records, and communications to clearly accountable elected officers.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

## Design principle — GSU leadership does not alter the Local Executive Board

### Draft 1 baseline

The elected GSU President serves as the principal liaison to Local 6350 leadership.

Draft 1 does **not** create a new Local 6350 officer or guaranteed voting Executive Board seat.

If Local leadership later wants a guaranteed GSU Executive Board seat, that should be considered separately as a Local-wide governance change.

---

## Decision 3 — One-year GSU terms
---

## Decision 3 — One-year GSU terms

### Proposal

GSU officers and Representative Council members serve annual terms, normally July 1 through June 30.

### Recommendation

**Approve as proposed.**

### Why

Graduate-worker turnover is much faster than the workforces covered by the comparator constitutions. The Local's three-year term is poorly suited to graduate employment and would predictably create vacancies.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

## Decision 4 — No one-year waiting period for GSU office

### Proposal

Any GSU member in good standing may run for GSU office or Representative Council.

The existing Local one-year membership requirement does not apply to GSU chapter office, .

### Recommendation

**Approve as proposed.**

### Why

A mandatory one-year waiting period substantially narrows leadership eligibility in a high-turnover bargaining unit and would make the initial GSU election impossible because the GSU itself has not existed for one year. The proposed amendment expressly creates the exception for GSU chapter office.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

## Decision 5 — Separate GSU Representative Council

### Proposal

Create an elected GSU Representative Council that is **separate from the Local 6350 Executive Board**.

GSU officers and college/school representatives sit on the Council.

No GSU chapter office automatically carries a Local Executive Board seat.

### Recommendation

**Approve as proposed.**

### Why

Chelmsford separates its Representative Council from its Executive Board. This gives broad internal representation without unintentionally changing the balance of Local-wide governance.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

## Decision 6 — College/school representation formula

### Proposal

Each college or school with at least five GSU members receives:

- one guaranteed Representative Council seat; plus
- one additional seat for every additional fifty GSU members.

The entire GSU membership also elects two at-large representatives.

For the first election, if reliable college/school membership counts are unavailable, use one representative per recognized college/school and begin proportional reapportionment the following election.

### Recommendation

**Approve the hybrid principle; adjust numerical thresholds if leadership prefers.**

### Why

It guarantees small academic units a voice while allowing larger colleges to receive additional representation.

**Leadership decision:** APPROVE / MODIFY / REJECT

Preferred threshold if modified:

---

## Decision 7 — RA/TA and MS/PhD interests handled through committees/caucuses

### Proposal

Do not create separate overlapping elected districts for:

- Master's vs. doctoral students;
- RAs vs. TAs;
- international graduate workers.

Instead, authorize GSU to establish standing committees/caucuses for these groups.

### Recommendation

**Approve as proposed.**

### Why

These identities overlap and can change between semesters. College/school constituencies provide a stable electoral base; committees provide formal representation for cross-cutting issues.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

## Decision 8 — GSU controls its own bargaining team

### Proposal

The Local Constitution authorizes each bargaining unit to select its own bargaining committee.

GSU selects its bargaining team and develops its bargaining proposals through its own democratic process.

Only GSU members vote on ratification of a GSU contract, preserving current Article XV.

### Recommendation

**Approve as proposed.**

### Why

Both Chelsea and Chelmsford give bargaining-unit-specific officers defined roles in negotiations. Local 6350 already limits contract ratification to the affected bargaining unit.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

## Decision 9 — Special elections for vacancies

### Proposal

- Vice President succeeds to GSU President.
- Other officer vacancies with more than 90 days remaining are filled by special election.
- Representative vacancies with more than 90 days remaining are normally filled by election of the affected constituency.
- Short end-of-term vacancies may be filled temporarily by the Representative Council.

### Recommendation

**Approve as proposed.**

### Why

This minimizes unelected leadership while avoiding unnecessary elections immediately before a term ends.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

## Decision 10 — Initial implementation

### Proposal

After constitutional adoption:

- nominations open within 14 days;
- the initial GSU election is conducted after the required election notice;
- elected officers take office immediately after certification;
- the initial term ends June 30, 2027;
- regular spring elections begin in 2027.

### Recommendation

**Approve as proposed.**

### Why

The election framework has been updated to incorporate the AFT-required democratic-election safeguards, including secret ballot, mailed notice for officer elections, observer rights, and election-record retention.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

## Decision 11 — Temporary summer / appointment-gap continuity

### Proposal

A temporary gap between covered Graduate Student Employees appointments — including a summer without a covered UMass Dartmouth appointment — does **not** by itself terminate GSU membership or eligibility to continue serving in elected GSU office when the member is expected to return to a covered Graduate Student Employees appointment in the immediately following academic term.

### Recommendation

**Approve as proposed.**

### Why

GSU terms run across the summer, but graduate funding and appointment patterns do not always do so. Without a continuity rule, an otherwise eligible elected officer could lose office solely because of an ordinary summer funding gap despite returning to the bargaining unit in the fall.

This rule does **not** cover graduation, permanent departure from UMass Dartmouth, or a member who is not expected to return to a covered appointment.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

## Decision 12 — Contract-ratification administration

### Proposal

For a GSU collective bargaining agreement, GSU elected leadership participates in calling, administering, and communicating the ratification process **in coordination with Local 6350 officers**.

Only members of the Graduate Student Employees bargaining unit vote on the agreement, as the current Local Constitution already requires.

### Recommendation

**Approve as proposed.**

### Why

This gives GSU direct control and responsibility over communication with its own bargaining unit while preserving Local 6350's umbrella legal and administrative role.

**Leadership decision:** APPROVE / MODIFY / REJECT

Notes:

---

# Discussion item — Local 6350 name

The first draft preserves the existing name **U-Mass Dartmouth Maintainers, Local 6350**.

Leadership does **not** need to change the name for this amendment package to work. However, the group may discuss whether it eventually wants to recommend a broader umbrella name now that Local 6350 contains both Maintainers and Graduate Student Employees.

No name change is proposed in Draft 1.

---

# Already resolved before the meeting

These are no longer open research questions:

- use **Graduate Student Employees** as the graduate-unit terminology;
- retain the existing Local 6350 name in Draft 1;
- preserve the existing Local Executive Board structure; the GSU President is a liaison, not a new Local officer;
- expressly exempt GSU chapter office from the Local's one-year prior-membership rule;
- use AFT-required democratic-election safeguards for GSU officer elections;
- use the clarified income-based dues treatment for Graduate Student Employees;
- follow existing Article XIII for amendment notice and the two-thirds Local membership vote, followed by required post-adoption submission.

The current draft also **preserves**, rather than changes, the existing Local-wide Executive Board seats held by Local-wide committee chairs.

---

# Desired meeting outcome

By the end of Tuesday's discussion, leadership should be able to say:

> We approve this structure in principle, subject to the changes noted on this sheet, and authorize the draft constitutional amendments and GSU chapter bylaws to be sent to the Local 6350 President for review.

That is the decision needed to move from internal design to formal constitutional review.
